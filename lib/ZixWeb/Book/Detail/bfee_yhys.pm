...
sub bfee_yhys {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_bfee_yhys $condition group by $fields";
...
sub bfee_yhys_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_bfee_yhys $condition group by $fields order by $fields";
