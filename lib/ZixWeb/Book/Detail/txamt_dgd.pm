...
sub txamt_dgd {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_txamt_dgd $condition group by $fields";
...
sub txamt_dgd_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_txamt_dgd $condition group by $fields order by $fields";
