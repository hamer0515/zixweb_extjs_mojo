package ZixWeb::Book::Detail::bsc_jf;
...
sub bsc_jf {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_bsc_jf $condition group by $fields";
...
sub bsc_jf_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_bsc_jf $condition group by $fields order by $fields";
