...
sub blc_zyzj {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_blc_zyzj $condition group by $fields";
...
sub blc_zyzj_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_blc_zyzj $condition group by $fields order by $fields";
