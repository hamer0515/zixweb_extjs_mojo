package ZixWeb::Book::Detail::txamt_yhyf;
...
sub txamt_yhyf {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_txamt_yhyf $condition group by $fields";
...
sub txamt_yhyf_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_txamt_yhyf $condition group by $fields order by $fields";
