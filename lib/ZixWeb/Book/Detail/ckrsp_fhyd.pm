package ZixWeb::Book::Detail::ckrsp_fhyd;
...
sub ckrsp_fhyd {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_ckrsp_fhyd $condition group by $fields";
...
sub ckrsp_fhyd_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_ckrsp_fhyd $condition group by $fields order by $fields";
