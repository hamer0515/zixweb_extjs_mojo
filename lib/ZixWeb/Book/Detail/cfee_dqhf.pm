package ZixWeb::Book::Detail::cfee_dqhf;
...
sub cfee_dqhf {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_cfee_dqhf $condition group by $fields";
...
sub cfee_dqhf_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_cfee_dqhf $condition group by $fields order by $fields";
