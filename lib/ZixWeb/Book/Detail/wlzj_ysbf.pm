package ZixWeb::Book::Detail::wlzj_ysbf;
...
#  index         => 1,
#  items         => {
#                     wlzj_type => "\x{5907}\x{4ED8}\x{91D1}\x{8D26}\x{53F7}id",
#                     period   => "\x{671F}\x{95F4}\x{65E5}\x{671F}",
#                   },
#  next_page     => 1,
#  params        => "&fir=wlzj_type&sec=period",
#  period        => undef,
#  prev_page     => 1,
#  sec           => "period",
#  total_page    => 1,
...
sub wlzj_ysbf {
    ...
    my $p = $self->params( { wlzj_type => $wlzj_type, 
                             period => [
    ...
    my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_wlzj_ysbf $condition group by $fields";
    warn $sql;
