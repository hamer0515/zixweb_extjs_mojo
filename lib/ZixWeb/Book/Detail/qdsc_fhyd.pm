package ZixWeb::Book::Detail::qdsc_fhyd;
...
sub qdsc_fhyd {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_qdsc_fhyd $condition group by $fields";
	use Data::Dump;
	Data::Dump->dump($sql);
...
sub qdsc_fhyd_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_qdsc_fhyd $condition group by $fields order by $fields";
