package ZixWeb::Book::Detail::qdlc_fhyd;
...
sub qdlc_fhyd {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d, rownumber() over(order by $fields) as rowid from sum_qdlc_fhyd $condition group by $fields";
...
sub qdlc_fhyd_excel {
	...
	my $sql =
"select $fields, sum(j) as j, sum(d) as d from sum_qdlc_fhyd $condition group by $fields order by $fields";
