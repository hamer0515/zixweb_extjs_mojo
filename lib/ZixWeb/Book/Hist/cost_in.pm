package ZixWeb::Book::Hist::cost_in;
...
sub cost_in {
	...
	my $sql =
"select id, c, p,  ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_cost_in $p->{condition}";
...
sub cost_in_excel {
	...
	my $sql =
"select id, c, p,  ys_id, ys_type, j, d, period from book_cost_in $p->{condition} order by id desc";
