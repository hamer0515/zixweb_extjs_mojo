package ZixWeb::BookMgr::Hist::bfee_yhys;
...
#                      ...
#                    },
#  book           => "\x{5E94}\x{6536}\x{94F6}\x{884C}-\x{5DF2}\x{6838}\x{5E94}\x{6536}\x{94F6}\x{884C}\x{624B}\x{7EED}\x{8D39}",
#  count          => 0,
#  d_from         => undef,
#  d_to           => undef,
#  data           => [],
...
sub bfee_yhys {
    ...
    unless ($tag) {
        ...
        my $sql =
    "select id, bfj_acct, zjbd_type, zjbd_date, ys_id, ys_type, j, d, period, rownumber() over() as rowid from book_$book $p->{condition}";
