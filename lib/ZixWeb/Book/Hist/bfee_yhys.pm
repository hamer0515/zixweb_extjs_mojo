package ZixWeb::Book::Hist::bfee_yhys;
...
sub bfee_yhys {
    ...
    my $id = $self->param('id');
    my $params = {};
    for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to zjbd_date_from zjbd_date_to zjbd_type bfj_acct/) {
        my $p = $self->param($_);
        undef $p if $p eq '';
        $params->{$_} = $p;
    }
    ...
    $p = $self->params(
        {
            ...
            zjbd_date => [
                0,
                $params->{zjbd_date_from} && $self->quote( $params->{zjbd_date_from} ),
                $params->{zjbd_date_to} && $self->quote( $params->{zjbd_date_to} )
            ],
        }
    );
    my $sql =
"select id, bfj_acct, ys_id, ys_type, j, d, period, zjbd_type, zjbd_date, rownumber() over(order by id desc) as rowid from book_bfee_yhys $p->{condition}";
