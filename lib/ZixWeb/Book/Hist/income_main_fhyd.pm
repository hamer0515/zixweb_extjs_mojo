...
sub income_main_fhyd {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to fyw_type fm fhw_type/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub income_main_fhyd_excel {
	...
	my $sql =
"select id, fyw_type, fm, fhw_type,  ys_id, ys_type, j, d, period from book_income_main_fhyd $p->{condition} order by id desc";
