package ZixWeb::Book::Hist::txamt_dgd;
...
sub txamt_dgd {
	...
	my $sql =
"select id, bi, ys_id, ys_type, j, d, period, tx_date, rownumber() over(order by id desc) as rowid from book_txamt_dgd $p->{condition}";
...
sub txamt_dgd_excel {
	...
	my $sql =
"select id, bi, ys_id, ys_type, j, d, period, tx_date from book_txamt_dgd $p->{condition} order by id desc";
