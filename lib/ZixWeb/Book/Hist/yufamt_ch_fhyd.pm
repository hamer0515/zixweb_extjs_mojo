...
sub yufamt_ch_fhyd {
	...
	my $params = {};
	for (
		qw/ys_type ys_id j_from j_to d_from d_to period_from period_to fyw_type fc /
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub yufamt_ch_fhyd_excel {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to fyw_type fc fio_date_from fio_date_to/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
