package ZixWeb::Book::Hist::bfj_cust;
...
sub bfj_cust {
    ...
    my $id = $self->param('id');
    my $params = {};
    for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to c /) {
        my $p = $self->param($_);
        undef $p if $p eq '';
        $params->{$_} = $p;
    }
    ...
    my $sql =
"select id, c, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_bfj_cust $p->{condition}";
