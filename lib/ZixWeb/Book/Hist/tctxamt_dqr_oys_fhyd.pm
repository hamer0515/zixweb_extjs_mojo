...
sub tctxamt_dqr_oys_fhyd {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to fhw_type fc ftx_date_from ftx_date_to fch_ssn f_rate/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub tctxamt_dqr_oys_fhyd_excel {
	...
	my $sql =
"select id, fhw_type, fc, ftx_date, fch_ssn, f_rate, ys_id, ys_type, j, d, period from book_tctxamt_dqr_oys_fhyd $p->{condition} order by id desc";
