package ZixWeb::Book::Hist::txamt_dqr_oyf;
...
sub txamt_dqr_oyf {
	...
	my $sql =
"select id, bi, tx_date, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_txamt_dqr_oyf $p->{condition}";
...
sub txamt_dqr_oyf_excel {
	...
	my $sql =
"select id, bi, tx_date, ys_id, ys_type, j, d, period from book_txamt_dqr_oyf $p->{condition} order by id desc";
