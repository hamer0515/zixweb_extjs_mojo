package ZixWeb::Book::Hist::cost_ncss_fhyd;
...
sub cost_ncss_fhyd{
	...
	my $id     = $self->param('id');
	my $params = {};
	for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to fc fhw_type f_ssn /)
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
		$params->{$_} = $p;
	...
	my $sql =
"select id,  fc, fhw_type,f_ssn, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_cost_ncss_fhyd $p->{condition}";
