package ZixWeb::Book::Hist::cost_dfss;
...
sub cost_dfss {
	...
	my $sql =
"select id, p,  ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_cost_dfss $p->{condition}";
...
sub cost_dfss_excel {
	...
	my $sql =
"select id, p,  ys_id, ys_type, j, d, period from book_cost_dfss $p->{condition} order by id desc";
