package ZixWeb::BookMgr::Hist::cost_dfss;
...
# result:
#{
#  book => "\x{6210}\x{672C}-\x{57AB}\x{4ED8}\x{635F}\x{5931}",
#  c => undef,
#  count => 0,
#  d_from => undef,
#  d_to => undef,
...
sub cost_dfss {
    ...
    unless ($tag) {
        if ( $data->{id} ) {
         ...
         my $sql =
    "select id, p, ys_id, ys_type, j, d, period, rownumber() over() as rowid from book_$book $p->{condition}";
