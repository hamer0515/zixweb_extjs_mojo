package ZixWeb::Book::Hist::cfee_dqhf;
...
sub cfee_dqhf {
    ...
    my $id = $self->param('id');
    my $params = {};
    for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to c cust_proto tx_date_to tx_date_from/) {
        my $p = $self->param($_);
        undef $p if $p eq '';
        $params->{$_} = $p;
    }
    ...
    $p = $self->params(
        {
            ...
            cust_proto => $params->{cust_proto} && $self->quote( $params->{cust_proto} ),
    ...
    my $sql =
"select id, c, cust_proto, tx_date, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_cfee_dqhf $p->{condition}";
