...
sub camt_dgd_fhyd {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to fyw_type fc ftx_date_from ftx_date_to fhw_type f_ssn fs_rate/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub camt_dgd_fhyd_excel {
	...
	my $sql =
"select id, fyw_type, fc, ftx_date, fhw_type,f_ssn, fs_rate, ys_id, ys_type, j, d, period from book_camt_dgd_fhyd $p->{condition} order by id desc";
