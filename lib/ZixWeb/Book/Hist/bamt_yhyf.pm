package ZixWeb::Book::Hist::bamt_yhyf;
...
sub bamt_yhyf {
    ...
    my $id = $self->param('id');
    my $params = {};
    for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to zyzj_acct zjbd_type zjbd_date_from zjbd_date_to/) {
        my $p = $self->param($_);
        undef $p if $p eq '';
        $params->{$_} = $p;
    }
    ...
    $p = $self->params(
        {
            ...
            zjbd_date => [0, $params->{zjbd_date_from} && $self->quote( $params->{zjbd_date_from}),
            $params->{zjbd_date_to} && $self->quote ($params->{zjbd_date_to})],
    ...
    my $sql =
"select id, zyzj_acct, zjbd_type, zjbd_date, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_bamt_yhyf $p->{condition}";
