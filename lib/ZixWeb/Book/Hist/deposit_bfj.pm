...
sub deposit_bfj {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to bfj_acct/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub deposit_bfj_excel {
	...
	my $sql =
"select id, bfj_acct, ys_id, ys_type, j, d, period from book_deposit_bfj $p->{condition} order by id desc"
	  ;
