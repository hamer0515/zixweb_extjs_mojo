package ZixWeb::Book::Hist::qdlc_fhyd;
...
sub qdlc_fhyd {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to fch fyw_type fe_date_from fe_date_to/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
	...
	my $sql =
"select id, fyw_type, fch, fe_date, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_qdlc_fhyd $p->{condition}";
...
sub qdlc_fhyd_excel {
	...
	my $sql = "select $fields from book_qdlc_fhyd $p->{condition}"
	  ;
