...
sub ckrsp_fhyd{
	...
	my $id     = $self->param('id');
	my $params = {};
	for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to fyw_type ftx_date_from ftx_date_to fhw_type f_ssn fs_rate/)
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
		$params->{$_} = $p;
	...
	$p = $self->params(
		{
			...
			fyw_type       => $params->{fyw_type} && $self->quote( $params->{fyw_type} ),
