package ZixWeb::Book::Hist::yfamt_ch_fhyd;
...
sub yfamt_ch_fhyd {
	...
	my $sql =
"select id, fyw_type, fc, ftx_date, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_yfamt_ch_fhyd $p->{condition}";
...
sub yfamt_ch_fhyd_excel {
	...
	my $sql =
"select id, fyw_type, fc, ftx_date, ys_id, ys_type, j, d, period from book_yfamt_ch_fhyd $p->{condition} order by id desc";
