...
sub yfamt_dcch_fhyd {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to f_dcn fyw_type fm ftx_date_from ftx_date_to/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub yfamt_dcch_fhyd_excel {
	...
	my $sql =
"select id, fyw_type, fm, f_dcn, ftx_date, ys_id, ys_type, j, d, period from book_yfamt_dcch_fhyd $p->{condition} order by id desc";
