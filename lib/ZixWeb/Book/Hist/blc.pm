...
sub blc {
	...
	my $params = {};
	for (
		qw/id ys_type ys_id j_from j_to d_from d_to period_from period_to e_date_from e_date_to zjbd_type bfj_acct/
	  )
	{
		my $p = $self->param($_);
		undef $p if $p eq '';
...
sub blc_excel {
	...
	my $sql =
"select id, bfj_acct, ys_id, ys_type, j, d, period, zjbd_type, e_date from book_blc $p->{condition} order by id desc";
