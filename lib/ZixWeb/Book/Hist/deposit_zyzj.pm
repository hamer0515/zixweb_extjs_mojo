package ZixWeb::Book::Hist::deposit_zyzj;
...
#  id            => undef,
#  index         => 1,
#  items         => { zyzj_acct => "\x{5907}\x{4ED8}\x{91D1}\x{8D26}\x{53F7}id" },
#  j_from        => undef,
#  j_to          => undef,
#  next_page     => 1,
#  params        => undef,
...
sub deposit_zyzj {
    ...
    my $id = $self->param('id');
    my $params = {};
    for (qw/ys_type ys_id j_from j_to d_from d_to period_from period_to zyzj_acct/) {
        my $p = $self->param($_);
        undef $p if $p eq '';
        $params->{$_} = $p;
    }
    ...
    my $sql =
"select id, zyzj_acct, ys_id, ys_type, j, d, period, rownumber() over(order by id desc) as rowid from book_deposit_zyzj $p->{condition}";
