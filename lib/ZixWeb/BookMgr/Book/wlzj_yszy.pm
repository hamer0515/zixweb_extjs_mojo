package ZixWeb::BookMgr::Book::wlzj_yszy;
...
#  count      => 4,
#  data       => [
#                  { d => 0, j => "0.60", period => "2013-03-24", rowid => 1 },
#                  ...
#                ],
#  header     => [
...
sub wlzj_yszy {
    ...
    unless ($tag) {
        ...
        my $sql =
    "select period, sum(j) as j, sum(d) as d, rownumber() over() as rowid from sum_wlzj_yszy $condition group by period";
