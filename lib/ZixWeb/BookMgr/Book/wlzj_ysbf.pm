package ZixWeb::BookMgr::Book::wlzj_ysbf;
...
#{
#  count      => 1,
#  data       => [{ d => 0, j => "1.00", period => "2013-05-06", rowid => 1 },...],
#  header     => [
...
sub wlzj_ysbf {
    ...
    unless ($tag) {
        ...
        my $sql =
    "select period, sum(j) as j, sum(d) as d, rownumber() over() as rowid from sum_wlzj_ysbf $condition group by period";
