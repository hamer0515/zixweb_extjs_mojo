package ZixWeb::BookMgr::Book::cost_dfss;
...
sub cost_dfss {
    ...
    unless ($tag) {
        ...
        my $sql =
    "select $fields, sum(j) as j, sum(d) as d, rownumber() over() as rowid from sum_cost_dfss $condition group by $fields";
