package ZixWeb::BookMgr::Hist::bfj_cust;
...
# result:
#{
#  book         => "\x{5BA2}\x{6237}\x{5907}\x{4ED8}\x{91D1}-\x{5907}\x{4ED8}\x{91D1}",
#  c            => undef,
#  count        => 740,
#  d_from       => undef,
#  d_to         => undef,
...
sub bfj_cust {
    ...
    unless ($tag) {
        ...
        my $sql =
    "select id, c, ys_id, ys_type, j, d, period, rownumber() over() as rowid from book_$book $p->{condition}";
