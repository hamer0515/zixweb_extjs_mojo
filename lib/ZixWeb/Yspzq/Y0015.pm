...
sub y0015 {
    ...
    my $data = {};
    for (qw/id flag bfj_acct_in bfj_acct_out period_from period_to revoke_user ts_revoke/) {
        $data->{$_} = $self->param($_);
    }
    if ( $data->{revoke_user} ) {
    ...
    my $p = $self->params(
        {
            ...
            id          => $data->{id},
            flag        => $data->{flag},
            bfj_acct_in => $data->{bfj_acct_in},
            bfj_acct_out => $data->{bfj_acct_out},
