...
sub y0144 {
	...
	my $data = {};
	for (
		qw/id flag  bi p clear_date_from clear_date_to period_from period_to revoke_user ts_revoke/
	  )
	{
		$data->{$_} = $self->param($_);
	}
	...
	my $sql =
"select id, flag, period, clear_date, bi, p, tx_amt, rownumber() over(order by id desc) as rowid from yspz_0144 $p->{condition}";
