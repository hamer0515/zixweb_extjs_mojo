...
sub y0018 {
	...
	my $data = {};
	for (
		qw/id flag period_from period_to revoke_user ts_revoke crt_user bfj_acct_bj/
	  )
	{
		$data->{$_} = $self->param($_);
	}
	...
	my $sql =
"select id, bfj_acct_bj, crt_id, tx_amt, flag, period, rownumber() over(order by id desc) as rowid from yspz_0018 $p->{condition}";
