...
sub y0030 {
    ...
    my $data = {};
    for (qw/id bi bfj_acct_bj zjbd_date_in_from zjbd_date_in_to c ssn tx_amt_from tx_amt_to flag period_from period_to revoke_user ts_revoke/) {
        $data->{$_} = $self->param($_);
    }
    if ( $data->{revoke_user} ) {
    ...
    my $p = $self->params(
        {
            ...
            zjbd_date_in => [
                0,
                $data->{zjbd_date_in_from} && $self->quote( $data->{zjbd_date_in_from} ),
                $data->{zjbd_date_in_to} && $self->quote( $data->{zjbd_date_in_to} )
            ],
            tx_amt   => [
                2,
