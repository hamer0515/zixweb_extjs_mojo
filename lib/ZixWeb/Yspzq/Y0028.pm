...
sub y0028 {
    ...
    my $data = {};
    for (qw/id ssn c wlzj_type p flag period_from period_to revoke_user ts_revoke/) {
        $data->{$_} = $self->param($_);
    }
    if ( $data->{revoke_user} ) {
    ...
    my $sql =
        "select id, c, cfee, cwws_cfee, flag, period, rownumber() over(order by id desc) as rowid from yspz_0028 $p->{condition}";
