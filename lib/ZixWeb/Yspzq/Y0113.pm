...
sub y0113 {
    ...
    my $data = {};
    for (qw/id flag bfj_acct_bj  bi period_from period_to revoke_user ts_revoke/) {
        $data->{$_} = $self->param($_);
    }
    if ( $data->{revoke_user} ) {
    ...
    my $sql =
        "select id, bi, bfj_acct_bj, tx_amt,flag, period, rownumber() over(order by id desc) as rowid from yspz_0113 $p->{condition}";
