...
sub add {
    ...
    $self->dbh->begin_work;
    my $user_sql =
'insert into tbl_user_inf(user_id, username, user_pwd, pwd_chg_date, status) values (nextval for seq_user_id, \''
      . $username
      . '\', \''
      . $password
      . '\', \''.$oper_date.'\', 1)';
...
sub check {
    ...
    my $id = $self->param('id');
    my $result = false;
    my $sql = "select count(*) as count from tbl_user_inf where username=\'" . $name . "\' and user_id <> $id";
