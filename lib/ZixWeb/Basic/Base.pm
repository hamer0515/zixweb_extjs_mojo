...
sub roles{
    my $self = shift; 
    my $id = $self->param('id');
    my $data = $self->select("select role_id from tbl_user_role_map where user_id = $id");
    my $result = [];
    for my $d(@$data){
...
sub allroles {
    my $self = shift;
    my $data = $self->select("select role_id, role_name as name from tbl_role_inf where status = 1");
...
sub routes{
    ...
    my $checked = {};
    if ($id) {
        my $cdata = $self->select("select route_id from tbl_role_route_map where role_id = $id");
        for my $r(@$cdata){
    ...
    for my $parent(@$parents){
        my $children = [grep {$_->{parent_id} && $_->{parent_id} == $parent->{route_id}} @$rdata];
        map { 
                delete $_->{parent_id};
                $_->{leaf} = 1;
...
sub books {
    ...
    for my $key (sort { $books->{$a}->[2] cmp $books->{$b}->[2] } keys %$books){
        push @$result, {id => $key, name => $books->{$key}[2].'-'.$books->{$key}[1]};
    }
...
sub zyzjacct {
    ...
    for my $key (sort {$zyzj_acct->{$a} cmp $zyzj_acct->{$b}} keys %$zyzj_acct){
...
sub zjbdtype {
    ...
    for my $key (sort {$zjbd_type->{$a} cmp $zjbd_type->{$b}} keys %$zjbd_type){
